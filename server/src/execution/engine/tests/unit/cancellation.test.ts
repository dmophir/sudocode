/**
 * Tests for Task Cancellation (ISSUE-059)
 *
...
describe('Task Cancellation (ISSUE-059)', () => {
  ...
  describe('Cancel Queued Task', () => {
    ...
    it('cancels correct task when multiple tasks queued', async () => {
      ...
      assert.strictEqual(blockedEngine.getTaskStatus('task-1')?.state, 'queued');
      assert.strictEqual(blockedEngine.getTaskStatus('task-2'), null);
      assert.strictEqual(blockedEngine.getTaskStatus('task-3')?.state, 'queued');
  ...
  describe('Cancel and Queue Processing', () => {
    it('starts next queued task after cancellation', async () => {
      ...
      assert.strictEqual(limitedEngine.getTaskStatus('task-1')?.state, 'running');
      assert.strictEqual(limitedEngine.getTaskStatus('task-2')?.state, 'queued');
  ...
  describe('Edge Cases', () => {
    ...
    it('cancels task with dependencies correctly', async () => {
      ...
      assert.strictEqual(blockedEngine.getTaskStatus('task-2')?.state, 'queued');
    });
  });
});
