...
export interface WorkerPoolEventHandlers {
  ...
  onCrash?: (executionId: string, exitCode: number | null, signal: string | null) => void
}
...
export class ExecutionWorkerPool {
  ...
  async startExecution(
    ...
    if (this.config.verbose) {
      console.log(`[WorkerPool:${this.projectId}] Starting worker ${workerId} for execution ${execution.id}`)
    }
    ...
    const workerProcess = fork(workerScript, [], {
      ...
      execArgv: isDev ? ['-r', 'tsx/cjs'] : [], // Use tsx in development
    })
  ...
  private setupWorkerHandlers(worker: WorkerProcess): void {
    ...
    workerProcess.on('message', (message: any) => {
      if (!isWorkerMessage(message)) {
        console.warn(`[WorkerPool:${this.projectId}] Invalid message from worker ${workerId}:`, message)
        return
      }
    ...
    workerProcess.on('error', (error) => {
      console.error(`[WorkerPool:${this.projectId}] Worker ${workerId} error:`, error)
   ...
   */
  private handleWorkerMessage(worker: WorkerProcess, message: WorkerToMainMessage): void {
    const { executionId, workerId } = worker

    switch (message.type) {
      case 'ready':
        if (this.config.verbose) {
          console.log(`[WorkerPool:${this.projectId}] Worker ${workerId} ready`)
        }
      ...
      case 'status':
        if (this.config.verbose) {
          console.log(`[WorkerPool:${this.projectId}] Worker ${workerId} status: ${message.status}`)
        }
        worker.status = 'running'
      ...
      case 'complete':
        if (this.config.verbose) {
          console.log(`[WorkerPool:${this.projectId}] Worker ${workerId} completed`)
        }
      ...
      case 'error':
        console.error(`[WorkerPool:${this.projectId}] Worker ${workerId} error:`, message.error)
        if (message.fatal) {
          worker.status = 'failed'
        }
        this.eventHandlers.onError?.(executionId, message.error, message.fatal ?? false)
        break
    }
  }
  ...
  private handleWorkerExit(
    ...
    if (this.config.verbose) {
      console.log(
        `[WorkerPool:${this.projectId}] Worker ${workerId} exited:`,
        { exitCode, signal, status: worker.status }
      )
    }
  ...
  async cancelExecution(executionId: string): Promise<void> {
    ...
    if (this.config.verbose) {
      console.log(`[WorkerPool:${this.projectId}] Canceling execution ${executionId}`)
    }
    ...
    try {
      ...
      console.warn(`[WorkerPool:${this.projectId}] Failed to send cancel message:`, error)
    }
    ...
    await new Promise<void>((resolve) => {
      const timeout = setTimeout(() => {
        if (worker.process.pid) {
          if (this.config.verbose) {
            console.log(`[WorkerPool:${this.projectId}] Force killing worker ${worker.workerId}`)
          }
  ...
  async shutdown(): Promise<void> {
    ...
    if (this.config.verbose) {
      console.log(`[WorkerPool:${this.projectId}] Shutting down pool with ${this.workers.size} active workers`)
    }

    // Kill all workers
    const killPromises = Array.from(this.workers.keys()).map((executionId) =>
      this.cancelExecution(executionId).catch((error) => {
        console.error(`[WorkerPool:${this.projectId}] Error killing worker:`, error)
      })
    )
