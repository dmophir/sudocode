...
class EventCollector {
  ...
  connect(): void {
    ...
    transport.handleConnection(this.connectionId, mockRes as any, this.connectionId);
  }
  ...
  getEventsForExecution(executionId: string): AgUiEvent[] {
    return this.events.filter((e: any) =>
      ...
      (e.data && (e.data.runId === executionId || e.data.executionId === executionId))
    );
  }
...
describe('Workflow Integration Tests', { skip: SKIP_E2E }, () => {
  ...
  before(() => {
    ...
    lifecycleService = new ExecutionLifecycleService(db, gitRepoPath, worktreeManager);
